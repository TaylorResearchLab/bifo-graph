...
def parse_args():
    ...
    p.add_argument('--provenance', default=None,
                   help='Path to filled-out analysis_provenance_template.yaml. '
                        'Copy config/analysis_provenance_template.yaml to your '
                        'results directory, fill in all REQUIRED fields, then '
                        'supply the path here. If omitted, a first-run message '
                        'is printed and RUN_PROVENANCE.md is written with '
                        'auto-filled pipeline parameters only.')
    return p.parse_args()
...
def write_llm(summary, seed_ids, reference_ids, cohort_name, disease,
              ...
              prov_data=None, prov_missing=None, prov_found=False, today=None):
    ...
    # --- FAIR Provenance and Reproducibility section ---
    L.append(
    ...
    _prov_data    = prov_data    or {}
    _prov_missing = prov_missing or []

    if prov_found and _prov_data:
        if _prov_missing:
            L.append(
                f'### Provenance Status: PARTIAL\n\n'
                f'**{len(_prov_missing)} required field(s) still null** — '
                'fill them in and re-run for full FAIR compliance.\n\n'
                '**Missing fields:** ' + ', '.join(_prov_missing) + '\n'
            )
        else:
            L.append('### Provenance Status: COMPLETE\n')
        L.append(_format_provenance_yaml(_prov_data, _prov_missing) + '\n')
    else:
        L.append(
            '### Provenance Status: INCOMPLETE\n\n'
            '**No provenance file was supplied.** '
            'Copy `config/analysis_provenance_template.yaml` to your results '            'directory, fill in all REQUIRED fields, and re-run with '            '`--provenance <path/to/provenance.yaml>`.\n\n'            '**Auto-recorded pipeline parameters:**\n\n'
            f'- Cohort: {cohort_name}\n'
            f'- Disease: {disease_str}\n'
            f'- Analysis date: {today}\n'
            f'- Scores file: {scores_path}\n'
            f'- N resolved seeds: {n_display}\n'
            f'- Pathways scored: {n_total}\n'
            f'- Significant pathways (q<0.05): {n_sig}\n'
...
# ---------------------------------------------------------------------------
# Provenance helpers — YAML-based
# ---------------------------------------------------------------------------

# Required fields in the provenance YAML (null = not filled in)
REQUIRED_FIELDS = [
    ('analysis',        'type'),
    ...
    ('cohort',          'repository'),
    ('cohort',          'accession'),
    ('cohort',          'data_access_url'),
    ('knowledge_graph', 'name'),
    ('knowledge_graph', 'version'),
    ('knowledge_graph', 'doi_or_url'),
    ('knowledge_graph', 'export_query_file'),
    ('pipeline',        'bifo_git_commit'),
    ('pipeline',        'bifo_ontology_version'),
    ('pipeline',        'bifo_ontology_file'),
    ('pipeline',        'pipeline_log_file'),
    ('attribution',     'analyst_name'),
    ('attribution',     'institution'),
    ('attribution',     'contact_email'),
]

FIRST_RUN_MSG = """
NOTE: No provenance file supplied.
For FAIR compliance, fill out the provenance template and re-run:

  cp config/analysis_provenance_template.yaml {outdir}/provenance.yaml
  # edit {outdir}/provenance.yaml  (fill in all REQUIRED fields)
  # then re-run summarize_results.py with:
  #   --provenance {outdir}/provenance.yaml

RUN_PROVENANCE.md has been written to {outdir} with auto-filled
pipeline parameters. It is incomplete until provenance.yaml is supplied.
"""


def load_provenance_yaml(path):
    """
    Load and validate a filled-out analysis_provenance_template.yaml.
    Returns (data, missing_required, found) where:
      data             : parsed dict (or {} if not found)
      missing_required : list of 'section.field' strings still null
      found            : bool — True if file was found and parsed
    """
    if not path:
        return {}, [], False
    try:
        ...
        print("WARNING: PyYAML not installed. Install with: pip install pyyaml",
              file=sys.stderr)
        print("         Falling back to plain-text provenance embedding.",
              file=sys.stderr)
        # Fall back: treat as plain text
        try:
            with open(path) as f:
                text = f.read()
            return {'_raw_text': text}, [], True
        except FileNotFoundError:
            return {}, [], False
    ...
    try:
        ...
        print(f"WARNING: provenance file not found: {path}", file=sys.stderr)
        return {}, [], False
    except Exception as e:
        print(f"WARNING: could not parse provenance YAML: {e}", file=sys.stderr)
        return {}, [], False

    # Auto-fill python_version
    if 'pipeline' not in data:
        data['pipeline'] = {}
    if not data['pipeline'].get('python_version'):
        data['pipeline']['python_version'] = sys.version.split()[0]
    ...
    for section, field in REQUIRED_FIELDS:
        ...
        elif val.get(field) is None:
            missing.append(f'{section}.{field}')

    return data, missing, True


def _format_provenance_yaml(data, missing):
    """
    Format parsed provenance YAML into a human-readable markdown block
    for embedding in RUN_PROVENANCE.md and pathway_results_llm.md.
    """
    if '_raw_text' in data:
        return data['_raw_text']

    def val(section, field, default='Not specified'):
        return str(data.get(section, {}).get(field) or default)
    ...
    lines += [
        ...
        f"  Disease:              {val('cohort','disease')}",
        f"  N probands:           {val('cohort','n_probands')}",
        f"  Source:               {val('cohort','source')}",
        f"  Repository:           {val('cohort','repository')}",
        f"  Accession:            {val('cohort','accession')}",
        f"  Data access URL:      {val('cohort','data_access_url')}",
        f"  Demographics:         {val('cohort','demographics','Not provided (optional)')}",
        f"  IRB / consent:        {val('cohort','irb_protocol','Not provided (optional)')}",
        "",
        "--- Knowledge Graph ---",
        f"  Name:                 {val('knowledge_graph','name')}",
        f"  Version:              {val('knowledge_graph','version')}",
        f"  DOI / URL:            {val('knowledge_graph','doi_or_url')}",
        f"  Export query file:    {val('knowledge_graph','export_query_file')}",
        f"  Export query text:    {val('knowledge_graph','export_query_text','Not provided (optional)')}",
        "",
        "--- Pipeline ---",
        f"  BIFO git commit:      {val('pipeline','bifo_git_commit')}",
        f"  BIFO ontology ver:    {val('pipeline','bifo_ontology_version')}",
        f"  BIFO ontology file:   {val('pipeline','bifo_ontology_file')}",
        f"  Pipeline log file:    {val('pipeline','pipeline_log_file')}",
        f"  score_pathways cmd:   {val('pipeline','score_pathways_command','See last_command.txt in results directory')}",
        f"  Python version:       {val('pipeline','python_version')}",
        "",
        "--- Attribution ---",
        f"  Analyst:              {val('attribution','analyst_name')}",
        f"  Institution:          {val('attribution','institution')}",
        f"  Contact:              {val('attribution','contact_email')}",
        f"  Publication DOI:      {val('attribution','publication_doi','Not provided (optional)')}",
    ]

    notes = data.get('notes', '')
    if notes and notes.strip() and not notes.strip().startswith('#'):
        lines += ["", "--- Notes ---", notes.strip()]

    if missing:
        lines += [
            "",
            f"INCOMPLETE FIELDS ({len(missing)} required fields still null):",
        ] + [f"  - {f}" for f in missing]

    return '\n'.join(lines)


def write_reproduce_md(args, prov_data, prov_missing, prov_found,
                       last_command, today):
    """
    Write RUN_PROVENANCE.md to args.outdir.
    Auto-fills all pipeline parameters. Embeds formatted provenance
    if --provenance was supplied; otherwise emits first-run instructions.
    """
    outpath = os.path.join(args.outdir, 'RUN_PROVENANCE.md')

    def ap(path_arg):
        return os.path.abspath(path_arg) if path_arg else 'Not specified'

    sections = [
        '# BIFO-PPR Per-Run Reproducibility Record',
        f'**Cohort:** {args.cohort_name}',
        f'**Date:** {today}',
        ...
        '',
        f'- Scores file:            `{ap(args.scores)}`',
        f'- Seeds file:             `{ap(args.seeds)}`',
        f'- Edges file:             `{ap(args.edges_merged)}`',
        f'- Nodes file:             `{ap(args.nodes)}`',
        f'- Reference file:         `{ap(args.reference)}`',
        f'- Member scores file:     `{ap(args.member_scores)}`',
        f'- Influential nodes file: `{ap(args.influential_nodes)}`',
        f'- Output directory:       `{ap(args.outdir)}`',
    ...
    if last_command:
        sections.append(f'- score_pathways command: `{last_command}`')

    sections += ['', '---', '', '## Data Provenance', '']

    if prov_found and prov_data:
        if prov_missing:
            sections.append(
                f'**PROVENANCE PARTIAL** — {len(prov_missing)} required '
                f'field(s) still null. Fill them in and re-run.'
            )
        else:
            sections.append('**PROVENANCE COMPLETE**')
        sections.append('')
        sections.append(_format_provenance_yaml(prov_data, prov_missing))
    else:
        outdir_abs = ap(args.outdir)
        # Try to load the template to embed it in the provenance record
        template_text = None
        # Look for template relative to this script's location
        import pathlib
        script_dir = pathlib.Path(__file__).parent
        template_candidates = [
            script_dir.parent / 'config' / 'analysis_provenance_template.yaml',
            pathlib.Path('config') / 'analysis_provenance_template.yaml',
        ]
        for tc in template_candidates:
            if tc.exists():
                template_text = tc.read_text()
                template_path = str(tc.resolve())
                break
        else:
            template_path = 'config/analysis_provenance_template.yaml'

        sections += [
            '**PROVENANCE INCOMPLETE** — No provenance file was supplied.',
            '',
            f'The provenance template is at: `{template_path}`',
            '',
            'Steps to complete provenance:',
            '',
            '```',
            f'cp {template_path} {outdir_abs}/provenance.yaml',
            f'# Edit {outdir_abs}/provenance.yaml — fill in all REQUIRED fields',
            f'# Then re-run:',
            f'# python3 pipeline/summarize_results.py --provenance {outdir_abs}/provenance.yaml ...',
            '```',
            '',
        ]
        # List the required fields so user knows what to fill in
        sections += [
            'The following REQUIRED fields must be filled in the template:',
            '',
        ] + [f'  - {s}.{f}' for s, f in REQUIRED_FIELDS]
        if not template_text:
            sections += [
                '',
                '_WARNING: Template file not found at expected location. '
                'Check that `config/analysis_provenance_template.yaml` '
                'exists in the bifo-graph repository root._',
            ]
...
def main():
    ...
    if not prov_found:
        print(FIRST_RUN_MSG.format(outdir=os.path.abspath(args.outdir)))
    elif prov_missing:
        print(f'WARNING: {len(prov_missing)} required provenance field(s) still null: '
              + ', '.join(prov_missing))

    # Read last_command.txt from results dir if present (written by score_pathways.py)
    last_command = None
    last_cmd_path = os.path.join(os.path.dirname(args.scores), 'last_command.txt')
    if os.path.exists(last_cmd_path):
        with open(last_cmd_path) as _f:
            last_command = _f.read().strip()

    write_reproduce_md(args, prov_data, prov_missing, prov_found,
                       last_command, today)
